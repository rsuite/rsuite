...
| onCreate           | (value: string[], item: [ItemDataType][item], event) => void                     | 创建新选项时的回调函数         |
...
| onSelect           | (value: string, item: [ItemDataType][item] , event) => void                      | 选择选项时的回调函数           |
...
| renderOption       | (label: ReactNode, item: [ItemDataType][item]) => ReactNode                      | 自定义选项渲染函数             |
| renderOptionGroup  | (groupTitle: ReactNode, item: [ItemDataType][item]) => ReactNode                 | 自定义选项组渲染函数           |
| renderValue        | (value: string[], items: [ItemDataType][item][], tags: ReactNode[]) => ReactNode | 自定义选中项渲染函数           |
| searchable         | boolean `(true)`                                                                 | 是否可以搜索                   |
| searchBy           | (keyword: string, label: ReactNode, item: [ItemDataType][item]) => boolean       | 自定义搜索匹配函数             |
