...
> 注意: `flexGrow` 不能同时设置 `width` 和 `onResize` 属性。 因为此时列宽完全受控，需要自己处理 `width` 的变化。
...
| children                 | (components: { Cell, HeaderCell, Column, ColumnGroup }) => ReactNode \| ReactNode | 接收参数 Cell、HeaderCell、Column、ColumnGroup 组件 - [使 TypeScript 使用更加方便](https://github.com/rsuite/rsuite-table#type-safety) |
| data \*                  | object[]                                                                          | 表格数据                                                                                                                               |
