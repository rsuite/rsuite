...
| block                | boolean                                                         | 堵塞整行                                                                     |             |
| calendarSnapping     | boolean                                                         | 当设置为 `true` 时，用户如果先选择右侧日历上的日期，则会自动切换到左侧日历上 | ![][5.69.0] |
...
| hideHours            | (hour:number, date:Date) => boolean                             | 隐藏指定的小时选项                                                           | ![][5.71.0] |
| hideMinutes          | (minute:number, date:Date) => boolean                           | 隐藏指定的分钟选项                                                           | ![][5.71.0] |
| hideSeconds          | (second:number, date:Date) => boolean                           | 隐藏指定的秒选项                                                             | ![][5.71.0] |
