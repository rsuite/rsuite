...
const IMPORT_FILE_CONFIG = {
  animation: ['animations.less', 'keyframes.less'],
  col: ['grid.less'],
  ...
  'date-picker': ['picker.less', 'calendar.less', 'date-picker.less'],
  'date-range-picker': [
    'picker.less',
    'calendar.less',
    'date-picker.less',
    'date-range-picker.less'
  ],
  drawer: ['drawer.less', 'animations.less', 'keyframes.less'],
...
(async function() {
  ...
  const jsFileNames = matchAll(jsContent, /export\s+(\w+)\s+from/gi).map(match =>
    kebabCase(match[1])
  );
