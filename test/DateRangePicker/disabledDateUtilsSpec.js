...
describe('DateRangePicker - disabledDate - utils', () => {
  ...
  it('allowedRange', () => {
    ...
    assert.equal(func(dayjs('2018-12-02')), false);
    assert.equal(func(dayjs('2018-11-01')), true);
    assert.equal(func(dayjs('2018-12-16')), true);
  });

  it('before', () => {
    const func = utils.before('2018-12-01');
    assert.equal(func(dayjs('2018-12-02')), false);
    assert.equal(func(dayjs('2018-12-01')), false);
    assert.equal(func(dayjs('2018-11-30')), true);
  });

  it('after', () => {
    const func = utils.after('2018-12-01');
    assert.equal(func(dayjs('2018-12-02')), true);
    assert.equal(func(dayjs('2018-12-01')), false);
    assert.equal(func(dayjs('2018-11-30')), false);
  });
