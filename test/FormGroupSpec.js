...
describe('FormGroup', () => {
  it('Should render a FormGroup', () => {
    let title = 'Test';
    let instance = ReactTestUtils.renderIntoDocument(<FormGroup>{title}</FormGroup>);
    assert.equal(findDOMNode(instance).className, 'rs-form-group');
    assert.equal(findDOMNode(instance).innerHTML, title);
  });

  it('Should have a validation State', () => {
    let instance = ReactTestUtils.renderIntoDocument(<FormGroup validationState="warning" />);
  ...
  it('Should have a error State', () => {
    let instance = ReactTestUtils.renderIntoDocument(<FormGroup isValid={false} />);
    assert.ok(findDOMNode(instance).className.match(/\bhas-error\b/));
  });

  it('Should have a success State', () => {
    let instance = ReactTestUtils.renderIntoDocument(<FormGroup isValid />);
    assert.ok(findDOMNode(instance).className.match(/\bhas-success\b/));
  });

  it('Should add size', () => {
    let instance = ReactTestUtils.renderIntoDocument(<FormGroup size="lg" />);
    assert.ok(findDOMNode(instance).className.match(/\bform-group-lg\b/));
  });

  it('Should have a custom className', () => {
    let instance = ReactTestUtils.renderIntoDocument(<FormGroup className="custom" />);
    assert.ok(findDOMNode(instance).className.match(/\bcustom\b/));
  });

  it('Should have a custom style', () => {
    const fontSize = '12px';
    let instance = ReactTestUtils.renderIntoDocument(<FormGroup style={{ fontSize }} />);
