...
describe('Button', () => {
  it('Should output a button', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button>Title</Button>);
  ...
  it('Should show the type if passed one', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button type="submit">Title</Button>);
    assert.equal(findDOMNode(instance).getAttribute('type'), 'submit');
  });

  it('Should output an anchor if called with a href', () => {
    const href = '/url';
    const instance = ReactTestUtils.renderIntoDocument(<Button href={href}>Title</Button>);
  ...
  it('Should call onClick callback', done => {
    ...
    const instance = ReactTestUtils.renderIntoDocument(<Button onClick={doneOp}>Title</Button>);
    ReactTestUtils.Simulate.click(findDOMNode(instance));
  });

  it('Should be disabled', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button disabled>Title</Button>);
  ...
  it('Should have block class', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button block>Title</Button>);
    assert.ok(findDOMNode(instance).className.match(/\bbtn-block\b/));
  });

  it('Should apply appearance', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button appearance="ghost">Title</Button>);
    assert.ok(findDOMNode(instance).className.match(/\bbtn-ghost\b/));
  });

  it('Should apply size class', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button size="lg">Title</Button>);
  ...
  it('Should default to shape="default"', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button shape="default">Title</Button>);
    assert.equal(instance.props.shape, 'default');
  });

  it('Should be active', () => {
    const instance = ReactTestUtils.renderIntoDocument(<Button active>Title</Button>);
  ...
  it('Should have a custom style', () => {
    const fontSize = '12px';
    const instance = ReactTestUtils.renderIntoDocument(<Button style={{ fontSize }} />);
