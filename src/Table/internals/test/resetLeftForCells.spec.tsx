...
describe('resetLeftForCells', () => {
  const makeCell = (width: number) => React.cloneElement(<Cell dataKey="x" />, { width, left: 0 });

  it('Should reset left positions starting at 0', () => {
    const cells = [makeCell(100), makeCell(200), makeCell(150)];
    const result = resetLeftForCells(cells) as React.ReactElement<any>[];
  ...
  it('Should add extraWidth to the last cell', () => {
    ...
    const result = resetLeftForCells(cells, 20) as React.ReactElement<any>[];
